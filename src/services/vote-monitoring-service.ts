...
export class VoteMonitoringService {
    ...
    private async extractUsername(): Promise<void> {
        try {
            ...
            if (usernameElement) {
                ...
                if (hrefAttribute && hrefAttribute.includes(PATHS.BIRI)) {
                    ...
                    if (username && username.length > 0) {
                        ...
                        this.loggingService.debug('Username saved to storage', { 
                            success: saveResult, 
                            username 
                        });
                ...
                if (fallbackElement) {
                    ...
                    if (fallbackHref) {
                        ...
                        if (fallbackUsername && fallbackUsername.length > 0) {
                            ...
                            this.loggingService.debug('Fallback username saved to storage', { 
                                success: saveResult, 
            ...
            this.loggingService.debug('Attempting to load username from storage as fallback');
            ...
            if (result.success && result.data) {
                ...
                this.loggingService.debug('Username loaded from storage', { username: this.userNick });
            } else {
    ...
    public async setUsername(username: string): Promise<boolean> {
        try {
            ...
            // Save to storage
            const saveResult = await storageService.setItem('userNick', username, StorageArea.LOCAL);
            this.loggingService.debug('Username manually saved to storage', { 
                success: saveResult, 
                username 
            });
